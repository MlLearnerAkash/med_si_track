...
# model.save("needle.pt")

# Load the trained .pt model
model = YOLO("/root/ws/med_si_track/runs/detect/train/weights/best.pt")  # Load the trained model in .pt format
...
results = model.train(project="custom_needle",
                      data="/mnt/data/needle_images_only/needle_images/YOLODataset_seg/dataset.yaml", 
                      epochs=1000, imgsz=2048, device=0, batch=15,
                      plots=False, resume=False, save_period=-1,
                      save=True, pretrained=False,
                      )
